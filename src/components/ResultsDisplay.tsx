...
import { CheckCircle, Download, ExternalLink, FileText, Copy, ChevronDown, ChevronUp } from 'lucide-react';
...
function extractMarketAndLink(line: string) {
  ...
  if (match) {
    return { market: match[1], variant: match[2], url: match[3] };
  }
...
export const ResultsDisplay = ({ result, onDownload }: ResultsDisplayProps) => {
  ...
  const copyAllLinks = () => {
    const allLinks =
      result.generatedLinks.length > 0
        ? result.generatedLinks.join('\n')
        : parsedLinks.map((link) => link.url).join('\n');
    navigator.clipboard.writeText(allLinks);
    toast({
      title: 'All links copied',
      description: `${result.generatedLinks.length > 0 ? result.generatedLinks.length : parsedLinks.length
    ...
    if (result && result.fileLocation) {
      ...
      if (match) {
        ...
        getResults(crqNumber, mode)
          .then(({ mainContent, validationSummary }) => {
            setFileContent(mainContent || 'Could not fetch file content.');
            setValidationSummary(validationSummary || null);
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <Button
                ...
                disabled={result.generatedLinks.length === 0 && parsedLinks.length === 0}
              >
          ...
          <div className="space-y-2 max-h-64 overflow-auto border rounded-lg p-3 bg-muted/30">
            {(result.generatedLinks.length > 0 ? result.generatedLinks : parsedLinks).length > 0 ? (
              (result.generatedLinks.length > 0 ? result.generatedLinks : parsedLinks).map(
                (linkObj, index) =>
                  typeof linkObj === 'string' ? (
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => copyToClipboard(linkObj)}
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => window.open(linkObj, '_blank')}
                          className="h-8 w-8 p-0"
                        >
                    ...
                    >
                      <span className="text-sm font-mono text-muted-foreground flex-1 mr-2">
                        {linkObj.market && <span className="font-semibold">[{linkObj.market}]</span>}{' '}
                        {linkObj.variant && <span>({linkObj.variant})</span>} {linkObj.url}
                      </span>
                      <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => copyToClipboard(linkObj.url)}
                          className="h-8 w-8 p-0"
                        >
                          <Copy className="w-3 h-3" />
                        </Button>
        ...
        {validationSummary && (
          <div className="mt-8 border-t pt-4">
            ...
            {showSummary && (
              <div className="mt-3 bg-gray-900 text-green-300 p-4 rounded-lg border border-green-700">
                <pre className="whitespace-pre-wrap text-sm">{validationSummary}</pre>
              </div>
            )}
          </div>
        )}
